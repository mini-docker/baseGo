...
func InitDb() {
	...
	str, err := utility.ExecPath()
	if err != nil {
		panic(fmt.Sprintf("ip2Region.db write failure2,%+v", err))
	}

	dbPath := filepath.Join(str, FileName)

	if !Exists(dbPath) {
		f, err := os.OpenFile(dbPath, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
		defer f.Close()
		if err != nil {
			panic(fmt.Sprintf("ip2Region.db write failure1,%+v", err))
		}
		_, err = f.Write(getData())
		if err != nil {
			panic(fmt.Sprintf("ip2Region.db write failure2,%+v", err))
		}
	}
...
func New() (*Ip2Region, error) {
	if asbPath == "" {
		return nil, errors.New("filepath of ip2region.db is not set!")
	}
