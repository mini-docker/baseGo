...
func (SystemLineService) AddSystemLine(lineId, lineName string, limitCost float64, mealId int, domain string, transType int, apiUrl, md5key, rsaPubKey, rsaPriKey string, status int) error {
	...
	// 写入redis
	cmd := conf.GetRedis().HSet(model.SYSTEM_LINE_REDIS_KEY, systemLine.LineId, string(lineByte))
	if cmd.Err() != nil {
		golog.Error("systemLineService", "EditSystemLine", "写入redis失败", cmd.Err())
	}
	// 设置超时时间6分钟
...
func (SystemLineService) EditSystemLineStatus(id, status int) error {
	...
	// 写入redis
	conf.GetRedis().HSet(model.SYSTEM_LINE_REDIS_KEY, systemLine.LineId, systemLine)
	return nil
}
