...
type (
	...
	Store interface {
		...
		Foreach(func(k string, v []byte) error) error
		Del(k string) error
	}
)
...
func New() (e *Echo) {
	...
	e.HTTPErrorHandler = e.YcyzDefaultHTTPErrorHandler
...
func (e *Echo) YcyzDefaultHTTPErrorHandler(err error, c Context) {
	...
	if he, ok := err.(*HTTPError); ok {
		code = he.Code
		msg = he.Msg
	} else if myErr, ok := err.(*Err); ok {
		if myErr.Code == 0 {
			myErr.Code = DEFAULT_CODE
		}
		if myErr.Msg == "" {
			myErr.Msg = err.Error()
		}
		if myErr.Msg == "" {
			errAppendMsg(c, myErr)
		}
	...
	if _, ok := msg.(string); ok {
		msg = Map{"msg": msg, "code": DEFAULT_CODE}
	}
...
ERROR:
	if code == http.StatusInternalServerError {
		golog.Error("echo.go", "DefaultHTTPErrorHandler", "error:%+v", err, "host", c.Request().URL)
	}
}
...
ERROR:
	golog.Error("echo.go", "DefaultHTTPErrorHandler", "%v", err, "host", c.Request().URL)
}
...
func NewHTTPError(code int, message ...interface{}) *HTTPError {
	he := &HTTPError{Code: code, Msg: http.StatusText(code)}
	if len(message) > 0 {
		he.Msg = message[0]
	}
	return he
...
func (he *HTTPError) Error() string {
	return fmt.Sprintf("code=%d, msg=%v", he.Code, he.Msg)
}
