...
func TestIntSliceDeduplication(t *testing.T) {
	type args struct {
		ints []int
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := IntSliceDeduplication(tt.args.ints); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("IntSliceDeduplication() = %v, want %v", got, tt.want)
			}
		})
	}
}
