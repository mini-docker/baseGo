...
func (engine *Engine) tbNameForMap(v reflect.Value) string {
	if v.Type().Implements(tpTableName) {
		return engine.TableMapper.Obj2Table(v.Interface().(TableName).TableName())
	}
...
func (engine *Engine) tbNameNoSchema(tablename interface{}) string {
	...
	case []interface{}:
		...
		if l > 0 {
			...
			case string:
				table = f.(string)
			case TableName:
				table = engine.TableMapper.Obj2Table(f.(TableName).TableName())
			default:
				v := rValue(f)
				t := v.Type()
	...
	case TableName:
		return engine.TableMapper.Obj2Table(tablename.(TableName).TableName())
	case string:
