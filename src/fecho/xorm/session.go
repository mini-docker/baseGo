...
func (session *Session) Table(tableNameOrBean interface{}) *Session {
	...
	arr := strings.Split(session.statement.AltTableName, "_")
	// Fixme 你这里没有考虑没有前缀的表名，导致云配置直接panic
	if len(arr) > 1 && arr[1] == "l" { //系统彩的表名不加前缀
		session.statement.AltTableName = session.statement.AltTableName[6:]
	}
	return session
}

func (session *Session) FullTable(tableNameOrBean interface{}) string {
	tableName := ""
	v := rValue(tableNameOrBean)
	t := v.Type()
	if t.Kind() == reflect.String {
		tableName = session.engine.TableMapper.Obj2Table(tableNameOrBean.(string))
	} else if t.Kind() == reflect.Struct {
		tableName = session.engine.tbNameForMap(v) //.tbName(v)
	}
	return tableName
}
