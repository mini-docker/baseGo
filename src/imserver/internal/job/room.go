...
func (r *Room) Push(op int32, msg []byte, lineId, agencyId string) (err error) {
	var p = &comet.Proto{
		Ver:      1,
		Op:       op,
		Body:     msg,
...
func (r *Room) pushproc(batch int, sigTime time.Duration) {
	...
	//log.Info( "Room", "pushproc", "start room:%s goroutine", r.id)
	...
	for {
		...
		_ = r.job.broadcastRoomRawBytes(r.id, buf.Buffer(), roomReadyProto.LineId, roomReadyProto.AgencyId)
	...
	r.job.delRoom(r.id)
	//log.Info( "Room", "pushproc", "room:%s goroutine exit", r.id)
}
...
func (j *Job) getRoom(roomID string) *Room {
	...
	if !ok {
		...
		j.roomsMutex.Unlock()
		//log.Info( "Job", "getRoom", "new a room:%s active:%d", roomID, len(j.rooms))
	}
	return room
}
