...
func (lb *LoadBalancer) Update(ins []*naming.Instance) {
	...
	if len(ins) == 0 || float32(len(ins))/float32(len(lb.nodes)) < 0.5 {
		log.Error("LoadBalancer", "Update", "load balancer update src:%d target:%d less than half", nil, len(lb.nodes), len(ins))
		return
	}
	lb.nodesMutex.Lock()
	for _, in := range ins {
		if old, ok := lb.nodes[in.Hostname]; ok && old.updated == in.LastTs {
			...
			if err != nil {
				log.Error("LoadBalancer", "Update", "instance(%+v) strconv.ParseInt(weight:%s) error(%v)", nil, in, meta[model.MetaWeight], err)
				continue
			}
			conns, err := strconv.ParseInt(meta[model.MetaConnCount], 10, 32)
			if err != nil {
				log.Error("LoadBalancer", "Update", "instance(%+v) strconv.ParseInt(conns:%s) error(%v)", nil, in, meta[model.MetaConnCount], err)
				continue
			}
