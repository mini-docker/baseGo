...
func (s *server) EstablishConn(ctx context.Context, req *pb.EstablishConnReq) (*pb.EstablishConnResp, error) {
	mid, key, roomId, hb, has, roomids, err := s.srv.EstablishConn(ctx, req.UserId, req.Base.Server, req.Base.Tokken, req.Base.LineId, req.Base.AgencyId)
	return &pb.EstablishConnResp{
		Result:    has,
		Mid:       mid,
		Heartbeat: hb,
		RoomID:    roomId,
...
func (s *server) JoinRoom(ctx context.Context, req *pb.JoinRoomReq) (*pb.JoinRoomeResp, error) {
	roomId, has, err := s.srv.CometGetJoinRoom(ctx, req.UserId, req.Base.Server)

	return &pb.JoinRoomeResp{
		Result: has,
		RoomID: roomId,
	}, err
}
