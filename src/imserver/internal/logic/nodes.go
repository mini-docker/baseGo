...
func (l *Logic) nodeAddrs(c context.Context, clientIP string) (domains, addrs []string) {
	...
	if err == nil {
		log.Error("Logic", "Logic", "error:", err)
		region = l.regions[province]
	}
	//log.Info( "Logic", "nodeAddrs", "nodeAddrs clientIP:%s region:%s province:%s domains:%v addrs:%v", clientIP, region, province, domains, addrs)
