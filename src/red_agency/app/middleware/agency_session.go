...
var (
	...
	ONE_HOURS        = time.Duration(1 * time.Hour)
)

// GetSession 根据sessionIdFull获取用户session信息
func (ss *SessionService) GetSession(sessionIdFull string) (*model.AgencySession, error) {
	// 获取session数据
	...
	var agencySession model.AgencySession
	err = json.Unmarshal([]byte(fResult), &agencySession)
	if err != nil {
		golog.Error("SessionService", "GetSession", "err:", err)
		return nil, err
	}
	return &agencySession, nil
}

// SaveSession 存储session信息
// listKey 用来存储所有sessionIdFull的redis list的key
func (ss *SessionService) SaveSession(listKey string, session *model.AgencySession) error {
	if session == nil {
