...
func AgencyAuthInit(next server.HandlerFunc) server.HandlerFunc {
	return func(c server.Context) error {
		// 获取cookie信息
		...
		if err == nil {
			...
			err = sessionService.SaveSession(model.RED_AGENCY_SESSION_LIST_KEY, session)
