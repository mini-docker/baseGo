...
func CreateOrdinaryRedPacket(redInfo *structs.RedPacket, gameTime int) error {
	...
	b, _ := json.Marshal(redLogList)
	_, err = conf.GetRedis().Get().Do("Set", logKey, string(b))
	_, err = conf.GetRedis().Get().Do("expire", time.Duration(gameTime)*time.Hour)
	// .Err()
	if err != nil {
		// 红包存储失败
		sess.Rollback()
	...
	AddTimeWheel(string(b), 0)

	// 转发im发送信息
	fmt.Println(data, "SendRoomMessageFuncdata")
		...
		if moneyInt == 0 {
			...
			if i > 1 {
				...
				if randNum > 0 {
					...
					if redMoney > moneyInt {
						moneyInt = 0
						redMoney = moneyInt
					} else {
						moneyInt = moneyInt - redMoney
					}
					info.ReceiveMoney = common.DecimalSum(common.DecimalDivs(float64(redMoney), 100), info.ReceiveMoney)
				}
			} else {
				info.ReceiveMoney = common.DecimalSum(common.DecimalDivs(float64(moneyInt), 100), info.ReceiveMoney)
			}
		}
